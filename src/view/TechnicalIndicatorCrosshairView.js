...
import { TooltipShowRule, LineStyle, CandleType } from '../data/options/styleOptions'
...
import {renderFillCircle, renderStrokeFillCircle} from '../renderer/circle'
...
export default class TechnicalIndicatorCrosshairView extends View {
  ...
  _draw () {
    ...
    if (kLineData) {
      ...
      const realDataPosY = isArea ? this._yAxis.convertToPixel(kLineData.close) : crosshair.y
      if (crosshair.paneId === this._additionalDataProvider.id()) {
        ...
        this._drawCrosshairLine(crosshairOptions, 'horizontal', realDataPosY, 0, this._width, renderHorizontalLine)
      }
      if (crosshair.paneId) {
        // 绘制十字光标垂直线
      ...
      if (isArea) {
        renderStrokeFillCircle(this._ctx, '#26ff00', '#000000', 1, { x: realDataPosX, y: realDataPosY }, 3)
      }
      this._drawTooltip(
